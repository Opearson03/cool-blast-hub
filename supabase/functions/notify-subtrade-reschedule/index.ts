 ...
 const handler = async (req: Request): Promise<Response> => {
   ...
   try {
     ...
     // For reschedule, group invites by batch_id (or individual id) to send ONE notification per recipient
     // Also generate fresh tokens for each group
     ...
     const inviteGroups: InviteGroup[] = [];
 
     if (body.action === "reschedule") {
       // Group by batch_id or individual id
       const groupMap = new Map<string, typeof invites>();
       for (const invite of invites) {
         const key = invite.batch_id || invite.id;
         if (!groupMap.has(key)) {
           groupMap.set(key, []);
         }
         groupMap.get(key)!.push(invite);
       }
 
       // Generate fresh tokens for each group
       for (const [groupKey, groupInvites] of groupMap) {
         const rawToken = generateToken();
         const tokenHash = await hashToken(rawToken);
         inviteGroups.push({ groupKey, invites: groupInvites, rawToken, tokenHash });
       }
     } else {
       // For cancel, just process individually
       for (const invite of invites) {
         inviteGroups.push({ 
           groupKey: invite.id, 
           invites: [invite], 
     ...
     for (const group of inviteGroups) {
       const primaryInvite = group.invites[0];
       ...
       if (body.action === "cancel") {
         ...
         // Reschedule: update tokens and reset status so contractor can re-confirm
         const newExpiresAt = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString();
         const now = new Date().toISOString();
 
         for (const invite of group.invites) {
           const updateData: Record<string, any> = {
             token_hash: group.tokenHash,
           ...
           // If this is a batch invite, also update batch_token_hash
           if (invite.batch_id) {
             updateData.batch_token_hash = group.tokenHash;
           }
           ...
           if (updateError) {
             console.error(`Failed to reset invite ${invite.id}:`, updateError);
             errors.push(`Failed to update invite for ${invite.recipient_name}`);
           }
         }
 
         console.log(`Reset ${group.invites.length} invites with new token for group ${group.groupKey}`);
       }
 
       // Build the invite URL using the fresh token (for reschedule)
       const inviteUrl = body.action === "reschedule" ? `${appUrl}/i/${group.rawToken}` : "";
 
       // Send SMS if phone exists (only need to send once per group)
       if (primaryInvite.recipient_phone && twilioAccountSid && twilioAuthToken && twilioPhoneNumber) {
         try {
           const formattedPhone = formatPhoneE164(primaryInvite.recipient_phone);
           ...
           if (!smsResponse.ok) {
             ...
             smsFailed++;
             errors.push(`SMS failed for ${primaryInvite.recipient_name}`);
           } else {
           ...
           smsFailed++;
           errors.push(`SMS error for ${primaryInvite.recipient_name}`);
         }
       }
 
       // Send email if email exists (only need to send once per group)
       if (primaryInvite.recipient_email && resendApiKey) {
         try {
       ...
       <td align="center">
         <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
           ...
           <tr>
             <td style="padding: 32px 24px;">
               ...
               <p style="color: #6b7280; margin: 0 0 24px 0; font-size: 16px;">
                 Hi ${primaryInvite.recipient_name},
               </p>
               <p style="color: #6b7280; margin: 0 0 24px 0; font-size: 16px;">
                 Unfortunately, your work invite for <strong>${oldDateFormatted}</strong> as <strong>${primaryInvite.role}</strong> has been cancelled.
               </p>
               <p style="color: #6b7280; margin: 0 0 24px 0; font-size: 16px;">
           ...
           <tr>
             ...
             const wasAccepted = primaryInvite.status === "accepted";
             emailHtml = `
 <!DOCTYPE html>
 <html>
 <head>
       ...
       <td align="center">
         <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
           ...
           <tr>
             <td style="padding: 32px 24px;">
               ...
               <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f9fafb; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                 <tr>
                   <td>
                     ...
                     <p style="margin: 0; color: #6b7280; font-size: 14px;">🔧 Role: <strong style="color: #1f2937;">${primaryInvite.role}</strong></p>
                   </td>
                 </tr>
               </table>
               ...
               <table width="100%" cellpadding="0" cellspacing="0">
                 <tr>
                   <td align="center">
                     <a href="${inviteUrl}" style="display: inline-block; background-color: #f97316; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">Confirm Availability</a>
                   </td>
                 </tr>
               </table>
             </td>
           ...
           const emailResponse = await resend.emails.send({
             ...
             to: [primaryInvite.recipient_email],
           ...
           if (emailResponse.error) {
             ...
             emailsFailed++;
             errors.push(`Email failed for ${primaryInvite.recipient_name}`);
           } else {
             console.log("Email sent to", primaryInvite.recipient_email);
           ...
           emailsFailed++;
           errors.push(`Email error for ${primaryInvite.recipient_name}`);
         }
       }
 
       // Update all invites in this group with delivery status and log audit events
       for (const invite of group.invites) {
         if (body.action === "reschedule") {
           await supabase
             .from("external_invites")
         ...
         // Log audit event for each invite
         ...
         await supabase.from("external_invite_events").insert({
           ...
           metadata: {
             ...
             token_rotated: body.action === "reschedule",
           },
         });
       }
     }
