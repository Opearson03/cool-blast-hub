...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (quoteTemplate === 'modern') {
      ...
      if (logoData) {
        try {
          const logoDims = calculateLogoDimensions(logoData, 50, 30);
          doc.addImage(`data:image/${logoData.format.toLowerCase()};base64,${logoData.base64}`, logoData.format, margin, yPos, logoDims.width, logoDims.height);
          yPos += logoDims.height + 5;
        } catch (e) {
          console.error("Failed to add logo to PDF:", e);
        }
      }
      ...
      // Logo and business name side by side (preserve aspect ratio)
      let logoWidth = 0;
      if (logoData) {
        try {
