import { LinearSection, HorizontalBarConfig, VerticalBarConfig, PriceMap } from "@/lib/estimate-components/types";
...
interface FootingTypeGroup {
  typeName: string;
  width: number;  // dimension1
  depth: number;  // dimension2
...
function parseFootingTypeName(name: string): string {
  // Extract base type name: "SF1-2" -> "SF1", "RWF1-3" -> "RWF1"
  const match = name.match(/^([A-Z]+\d+)/i);
  if (match) return match[1].toUpperCase();
  
  // For legacy names like "Strip Footing 1", use the full name as type
  return name.split('-')[0].trim();
}

function groupFootingsByType(sections: LinearSection[]): FootingTypeGroup[] {
  ...
  sections.forEach(section => {
    ...
    if (!groupMap.has(key)) {
      groupMap.set(key, {
        ...
        tm_type_top: section.tm_type_top,
        tm_price: section.tm_price,
        tm_price_top: section.tm_price_top,
        add_ligs: section.add_ligs,
        lig_size: section.lig_size,
        lig_centres: section.lig_centres,
        horizontal_bars: section.horizontal_bars,
        vertical_bars: section.vertical_bars,
        chairs_enabled: section.chairs_enabled,
        chair_type: section.chair_type,
        chairs_per_m: section.chairs_per_m,
        chair_price_per_bag: section.chair_price_per_bag,
      ...
      group.totalLength += section._actualLength || section.length || 0;
    }
  });
...
export function FootingSectionReinforcementInput({
  sections,
  onChange,
  defaultTmType,
  defaultAddLigs,
  ...
  label,
  priceMap,
}: FootingSectionReinforcementInputProps) {
  ...
  const addHorizontalBarToGroup = useCallback((group: FootingTypeGroup) => {
    ...
    const updatedSections = sections.map(section => {
      ...
      if (sectionType === group.typeName && width === group.width && depth === group.depth) {
        const currentBars = section.horizontal_bars || [];
        return { ...section, horizontal_bars: [...currentBars, newBar] };
      }
      return section;
    });
    onChange(updatedSections);
  ...
  const updateHorizontalBarForGroup = useCallback((group: FootingTypeGroup, barIndex: number, updates: Partial<HorizontalBarConfig>) => {
    const updatedSections = sections.map(section => {
      ...
      if (sectionType === group.typeName && width === group.width && depth === group.depth) {
        ...
        return { ...section, horizontal_bars: currentBars };
      }
      return section;
    });
    onChange(updatedSections);
  ...
  const addVerticalBarToGroup = useCallback((group: FootingTypeGroup) => {
    ...
    const updatedSections = sections.map(section => {
      ...
      const depth = section.dimension2 || 0;
      if (sectionType === group.typeName && width === group.width && depth === group.depth) {
        const currentBars = section.vertical_bars || [];
        return { ...section, vertical_bars: [...currentBars, newBar] };
      }
      return section;
    });
    onChange(updatedSections);
  ...
  const updateVerticalBarForGroup = useCallback((group: FootingTypeGroup, barIndex: number, updates: Partial<VerticalBarConfig>) => {
    const updatedSections = sections.map(section => {
      ...
      if (sectionType === group.typeName && width === group.width && depth === group.depth) {
        ...
        return { ...section, vertical_bars: currentBars };
      }
      return section;
    });
    onChange(updatedSections);
  ...
  useEffect(() => {
    ...
    const updatedSections = sections.map(section => {
      ...
      if (tmType !== 'none' && tmLayers > 1 && section.tm_price_top === undefined) {
                  ...
                  <div className="px-3 pb-3 pt-2 border-t bg-muted/30 space-y-4">
                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      {group.segments.map((s, i) => (
                        <span key={s.id}>
                          {s.name} ({(s._actualLength || s.length).toFixed(1)}m){i < group.segments.length - 1 ? ', ' : ''}
                        </span>
                      ))}
                    </div>
