...
import { MultiWafflePodZoneInput } from "./MultiWafflePodZoneInput";
  ...
  const derivedScopeAnswers = useMemo(() => {
    ...
    if (pierGroupsArray) {
      ...
      excavationVolume = pierGroupsArray.reduce((sum: number, group: any) => {
        ...
        const radiusM = (Number(group.diameter) || 0) / 2000; // mm to m
        const depthM = (Number(group.depth) || 0) / 1000; // mm to m
      ...
      excavationArea = pierGroupsArray.reduce((sum: number, group: any) => {
        const qty = Number(group.quantity) || 1;
        const radiusM = (Number(group.diameter) || 0) / 2000;
    ...
    // IMPORTANT: excavationArea stays as totalArea (for bulk excavation), detailed excavation uses beam volumes
      ...
      // For slab scopes, excavationArea = totalArea (for bulk excavation / site cuts)
      // Detailed excavation for beams is tracked in excavationVolume
      excavationArea = totalArea;
      ...
      if (edgeBeams.length > 0) {
        ...
        // Weighted average depth from edge beams
