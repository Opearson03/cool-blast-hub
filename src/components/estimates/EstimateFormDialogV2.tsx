...
interface EstimateFormDialogV2Props {
  open: boolean;
...
const { 
  ...
  type SaveDraftOptions = {
    /**
     * When true, closes the EstimateFormDialogV2 after saving.
  ...
  const renderScopeCalculator = (scope: ScopeType) => {
    ...
    if (hasMarkup) {
      ...
      if (slabScopesWithBeams.includes(scope) && scopeDefinition.supportsMultipleAreas) {
        ...
        if (raftSlabAreas.length > 0) {
          ...
          if (needsTakeoffMerge) {
            ...
            if (scope === 'waffle_pod') {
              ...
              const podSizeM = 1.09;
              const ribWidth = 0.11;
              const effectiveGridM = podSizeM + ribWidth;
        ...
        if (demolitionAreas.length > 0) {
          ...
          if (!hasUserData) {
            const areasFromTakeoff2 = demolitionAreas.map((area) => ({
              id: area.id,
              name: area.name,
              length: area.length,
              width: area.width,
            ...
            initialScopeAnswers = { ...initialScopeAnswers, _fromTakeoff: true };
            initialModuleAnswers = {
              ...initialModuleAnswers,
              demolition: {
                ...((initialModuleAnswers as any).demolition || {}),
                demolition_required: true,
                demolition_areas: areasFromTakeoff2,
              },
            };
          }
        }
        ...
        if (!hasUserData) {
          const areasFromTakeoff2 = scopeMarkups.map((markup, index) => {
            ...
            return {
              ...
              _fromTakeoff: true,
              _actualArea: area,
              _actualPerimeter: perimeter,
            };
          });
          initialScopeAnswers = { ...initialScopeAnswers, _fromTakeoff: true, areas: areasFromTakeoff2 };
        }
        ...
        if (pierConfigs.length > 0) {
          ...
          if (!hasUserData) {
            ...
            initialScopeAnswers = { ...initialScopeAnswers, _fromTakeoff: true, pierGroups };
          }
        }
        ...
        if (padConfigs.length > 0) {
          ...
          if (!hasUserData) {
            ...
            initialScopeAnswers = {
              ...initialScopeAnswers, _fromTakeoff: true,
              total_num_pads: totalCount,
              total_length: firstConfig.length, total_width: firstConfig.width, total_depth: firstConfig.depth,
        ...
        if (footingConfigs.length > 0) {
          const hasUserFootings = initialScopeAnswers.footings?.some((f: any) => (Number(f.length) || 0) > 0 && f._fromTakeoff !== true);
          const hasUserLinearSections = initialScopeAnswers.linearSections?.some((s: any) => (Number(s.length) || 0) > 0 && s._fromTakeoff !== true);
          if (!hasUserFootings && !hasUserLinearSections) {
            ...
            let weightedDim1 = 0, weightedDim2 = 0;
            if (totalLength > 0) {
            ...
            initialScopeAnswers = {
              ...initialScopeAnswers, _fromTakeoff: true,
              linearSections: linearSectionsFromTakeoff, footings: footingConfigs,
              total_length: totalLength, width: weightedDim1, depth: weightedDim2,
              footing_width: weightedDim1, footing_depth: weightedDim2,
              wall_thickness: weightedDim1, wall_height: weightedDim2,
            };
          }
        }
      } else {
    ...
    // Show loading state while takeoff data is being fetched
    if (takeoffLoading) {
      return (
        <Card className="p-8">
