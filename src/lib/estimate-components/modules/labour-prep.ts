...
export const labourPrepModule: EstimateModule = {
  ...
  questions: [
    {
      id: 'crew_size',
      type: 'number',
      label: 'How many workers for prep?',
      min: 1,
      max: 20,
      required: true,
      placeholder: 'Enter crew size',
      helpText: 'Total number of workers for preparation works',
    },
    {
      id: 'total_hours',
      type: 'number',
      label: 'Total prep hours',
      min: 0.5,
      step: 0.5,
      unit: 'hrs',
      required: true,
    ...
    },
    {
      ...
      priceListKey: 'labour.LABOUR PREP LEAD',
      unit: '/hr',
      required: true,
  ...
  calculate: (answers, priceMap, _scopeData): ComponentCost => {
    ...
    const leadRate = Number(answers.lead_rate) || getPrice(priceMap, 'labour', 'LABOUR PREP LEAD', 95);
    const useMixedRates = answers.use_mixed_rates === true;
    const crewRate = Number(answers.crew_rate) || getPrice(priceMap, 'labour', 'LABOUR PREP HR', 75);
    ...
    if (useMixedRates && crewSize > 1) {
      ...
      lineItems.push({
        id: 'lead_labour_prep',
        description: `Lead Worker - Prep (${totalHours} hrs @ $${leadRate}/hr)`,
        quantity: totalHours,
        unit: 'hrs',
        unitPrice: leadRate,
        total: leadCost,
      ...
      lineItems.push({
        id: 'crew_labour_prep',
        description: `Crew Labour - Prep (${otherWorkers} workers × ${totalHours} hrs @ $${crewRate}/hr)`,
      ...
      lineItems.push({
        id: 'regular_labour_prep',
        description: `Labour - Prep (${crewSize} workers × ${totalHours} hrs @ $${leadRate}/hr)`,
    ...
    if (answers.overtime_required) {
      ...
      if (overtimeHours > 0) {
        ...
        lineItems.push({
          id: 'overtime_labour_prep',
          description: `Overtime - Prep (${crewSize} workers × ${overtimeHours} hrs @ ${overtimeMultiplier}x)`,
  ...
  validate: (answers) => {
    ...
    if (!answers.crew_size || answers.crew_size < 1) {
      errors.push('Please specify the prep crew size');
    }
    if (!answers.total_hours || answers.total_hours < 0.5) {
      errors.push('Please specify the prep hours');
    }
